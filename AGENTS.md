...
- **A2A** (`@occ/a2a`): `occ-a2a --agent <id> --port N`; agent cards generated from the capability profile; `SendMessage` / `SendStreamingMessage` (SSE) / `GetTask` / `ListTasks` / `CancelTask` / `SubscribeToTask`, cards advertise `streaming: true`. Tasks are server-side state — clients that disconnect mid-run re-attach via `ListTasks` + `GetTask` (see README "Long tasks and re-attachment"). `http.ts` normalizes two `@a2a-js/sdk` wire quirks at the boundary (absent `status` filter decoding to `TASK_STATE_UNSPECIFIED` and breaking `ListTasks`; `"user"`/`"agent"` roles decoding to `UNRECOGNIZED`) — keep those shims; regression tests live in `packages/a2a/test/http.test.ts`.
